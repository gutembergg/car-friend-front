...
const AddPassenger = ({ carId, carSeats, getPassengerInCar, getCars, eventId, userPassEvent, userPassengerInEvent, userEvents }) => {
    ...
    async function handlePassenger(e) {
        ...
        try {
            ...
            if(user) {
                const userPassengerInEvent = userPassEvent.filter(passUser => passUser._id === eventId );
            ...
            if(token) {  
                await api.post(`/user/${eventId}/passengerInEvent`, {}, {
                    headers: {
            ...
            {showInputAddPassenger &&    
                <form data-toggle="validator">        
                    <div className="row">
                        ...
                        <div >
                            <MDBView hover onClick={handlePassenger} className={`${isButtonDisabled}`}>
                                <MDBIcon icon="check" style={{color: "grey"}} className="mt-4 ml-4" />                
                        ...
                        <div>
                            ...
                            <MDBIcon icon="times"  style={{color: "grey"}} className="mt-4 ml-3" />
                                    <MDBMask className="flex-center" overlay="white-strong"> 
                                    </MDBMask>
                            </MDBView>
                        </div>
